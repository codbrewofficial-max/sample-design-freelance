...
export async function generateMetadata(): Promise<Metadata> {
  return generatePageMetadata('galeri');
}

export default function GaleriPage() {
  const config = getSiteConfig();
  
  // Find the 'galeri' page configuration
  const pageConfig = config.pages.find((page) => page.slug === 'galeri');

  if (!pageConfig) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white font-mono p-6">
        <p>[Error: Galeri page configuration not found in site.config.json]</p>
      </div>
    );
  }

  // Load frontmatter data & markdown body content for each section on the galeri page
