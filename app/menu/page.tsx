...
export default function MenuPage() {
  ...
  // Find the 'menu' page configuration
  const pageConfig = config.pages.find((page) => page.slug === 'menu');

  if (!pageConfig) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white font-mono p-6">
        <p>[Error: Menu page configuration not found in site.config.json]</p>
      </div>
    );
  }

  // Load frontmatter data & markdown body content for each section on the menu page
