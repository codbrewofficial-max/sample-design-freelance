...
export default function HomePage() {
  ...
  const homePage = config.pages.find((page) => page.slug === 'home');

  if (!homePage) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white font-mono p-6">
        <p>[Error: Home page configuration not found in site.config.json]</p>
      </div>
    );
  }

  // Load frontmatter data & markdown body content for each section on the home page
  const resolvedSections = homePage.sections.map((sec) => {
